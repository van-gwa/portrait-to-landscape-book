...
$rowCounts = @{}
for ($r = 0; $r -lt $rowCount; $r++) { $cnt = 0; for ($c = 0; $c -lt $colCount; $c++) { if ($cells.containsKey("$r-$c")) { $cnt++ } }; if ($cnt -gt 0) { $rowCounts[$r] = $cnt } }
$colCounts = @{}
for ($c = 0; $c -lt $colCount; $c++) { $cnt = 0; for ($r = 0; $r -lt $rowCount; $r++) { if ($cells.containsKey("$r-$c")) { $cnt++ } }; if ($cnt -gt 0) { $colCounts[$c] = $cnt } }
